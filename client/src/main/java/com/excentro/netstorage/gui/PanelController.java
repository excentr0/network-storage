...
public class PanelController implements Initializable {
  ...
  public void initialize(final URL location, final ResourceBundle resources) {
    ...
    fileDateColumn.setCellValueFactory(
        param ->
            new SimpleStringProperty(param.getValue().getLastModified().format(timeFormatter)));
          ...
          if (event.getClickCount() == 2) {
            Path path =
                Paths.get(pathField.getText())
                    .resolve(localFiles.getSelectionModel().getSelectedItem().getFilename());
  ...
  public void updateFiles(Path path) {
    ...
    localFiles.getItems().clear(); // чистим список файлов
  ...
  public void selectDiskAction(final ActionEvent actionEvent) {
    ...
    updateFiles(Paths.get(element.getSelectionModel().getSelectedItem()));
  }
